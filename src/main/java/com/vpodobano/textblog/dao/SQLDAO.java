package com.vpodobano.textblog.dao;
...
import com.vpodobano.textblog.dao.mapper.ArticleMapper;
import com.vpodobano.textblog.dao.mapper.CommentMapper;
import com.vpodobano.textblog.dao.mapper.ListMapper;
import com.vpodobano.textblog.dao.mapper.MapCategoryMapper;
...
public final class SQLDAO {
	...
	public List<Article> listArticles(Connection c, int offset, int limit) throws SQLException {
		return sql.query(c, "select * from article a order by a.created desc limit ? offset ?", new ListMapper<>(new ArticleMapper()), limit, offset);
	}
	...
	public List<Comment> listComments(Connection c, long idArticle, int offset, int limit) throws SQLException {
		return sql.query(c, "select c.*, a.name, a.email, a.created as accountCreated, a.avatar from " +
				" comment c, account a where a.id=c.id_account and c.id_article=? order by c.created desc limit ? offset ?",
	...
	public Account createNewAccount(Connection c, String email, String name, String avatar) throws SQLException {
		return sql.insert(c, "insert into account(id,email,name,avatar) values(nextval('account_id_seq'),?,?,?)", 
	...
	public Comment createComment(Connection c, CommentForm form, long idAccount) throws SQLException {
		return sql.insert(c, "insert into comment(id, id_article,id_account,content) values(nextval('comment_id_seq'),?,?,?)", 
